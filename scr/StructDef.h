...
typedef struct
{
	...
	unsigned char card_catalog_bak;
...
typedef struct 
{
	...
	unsigned char tcpBuffer[490];		//最大可发送7个记录
}TCP_IP_PACKET1;
...
typedef struct  //采集时的数据文件信息
{	
	unsigned char yy[4];						//年（ASCII）
	unsigned char mm[2];						//月（ASCII）
	unsigned char dd[2];						//日（ASCII）
	unsigned char hh[2];						//时（ASCII）
	unsigned char m[2];						//分（ASCII）
	unsigned char s[2];							//秒（ASCII）
	unsigned char device_no[9];					// 设备号(ASCII)
	unsigned char package_serial_no[5];			//采集包序列号	(ASCII)
	unsigned char consume_flag;			//消费标志
	//累积金额和累积扣次全为零:consume_flag=0
	//否则consume_flag=1
	unsigned char sum_money[4];			//累积消费金额
	unsigned char sum_count[4];			//累计扣次
	unsigned char offset[2];			//页内偏移量
	unsigned char page;					//当前数据文件起始页号
	unsigned char length[4];			// 当前记录文件长
}Current_Information;
...
{	
	...
	char m[2];					//分（ASCII）                																					 
	char s[2];					       //秒（ASCII）                																					 
	char device_no[9];				// 设备号(ASCII)          
	char package_serial_no[5];		//采集包序列号	(ASCII)   
	
	unsigned char offset[2];	//页内偏移量
	char page;					//历史数据文件起始页号
	unsigned char length[4];	// 历史数据文件长
	//char  file_count;				//历史文件数
}History_Information;
typedef struct  
{	
	...
	char RecLength[5];	//每条记录长度,ASCII
}Rec_Information;//写入记录文件第一行，写入字节为记录长度，后面补空格
//***************************************************************************
//                       公共扇区的格式定义 
//***************************************************************************
...
typedef struct
{
	...
	unsigned char busTacVer[2]; //对称密钥信息版本2B
	unsigned char busVoiceVer[2];//公钥信息版本2B   如果有，需要转成16进制数
	unsigned char WHT_Bank[2];		//银联白名单版本
//	unsigned char MAC_KEY[2];		//羊城通计算前面MAC 用的key版本，用于和后台保持下发
//	unsigned char UIID[2];		//羊城通UIID版本，用于和后台保持下发
	unsigned char	QPOCVer[2];
	unsigned char	Linux4GVer[2];
...
typedef struct
{
	unsigned char busticketVer[2];//低在后
...
typedef struct{
	unsigned char ADRR[4];//C更新的地址
	unsigned char ALL_Pakge[4];//总个数
	unsigned char aldy_Pakge[4];//已经更新的个数
	unsigned char price_ver[2];//下载版本号
	unsigned char CRC[4];//CRC
}stbusdownFDinfo;//下载分段参数 	31
#define Fd_DownProLEN	18
typedef struct{
	unsigned char Miss_cmdCode[2];//CMDCODE，说明此数组当前功能，黑名单、线路 分段票价信息
	unsigned char Miss_style;//任务类型，GnCode
	unsigned int MISS_MAXPack;//最大包号
	unsigned char Miss_ver[2];//下载版本号
	unsigned int Miss_ALL_LEn;//需下载总字节数
...
#define Gmode_900C	0			/**/
#define Gmode_MG323 1			/*GPRS*/
#define Gmode_MC323 2			/*电信2G*/
#define Gmode_OTHER 3			/*外部传输，带协议*/
#define Gmode_WIFI_TLH 4		/*WIFI*/
#define Gmode_OTHER_TC 5		/*外部传输，不带协议*/
#define Gmode_CDMA2000 6		/*电信3G*/
#define Gmode_WCDMA 7			/*联通3G*/
#define Gmode_ALL4G 8			/*全网通4G*/
#define Gmode_YYLinux 9			/*移远Linux全网通4G*/
#define Gmode_MODEM 0x80
#define Gmode_Close 0xFF	//禁用GPRS
...
typedef struct{
	...
	unsigned short usRDNo;		//路段编号	(站点编号 )
	unsigned short usTIme1;		//时段一	（限制速度）
...
__packed typedef struct{
	...
	unsigned int uiHallfLiCeng;	//30分钟里程
	...
	//****************
	unsigned char  StarStation;	//超速开始站点
	unsigned short MaxYueSpeed;	//越站最高速度
	unsigned short MaxSpeed;	//超速最高速度
	unsigned short LimitSpeed;	//限制速度
...
typedef struct  
{
	unsigned char startdate[4];		//下发下来的日期4 YYYYMMDD 也就是文件明的一部分 发送完一个文件自增一次 也就是1日
	unsigned char enddate[4];		//下发下来的日期4 YYYYMMDD 
	unsigned char sfilename[4];		//正在发送文件名
