...
//	unsigned char buf[RECORD_LEN+32];
	...
	if (rfile_Wrecord_point >= 14)		//最后的2块不要用了，万一溢出了 因为银联和普通记录的长度不一样
	{
		MSG_LOG("满:%d\r\n",rfile_Wrecord_point);
		//debugstring("满擦--\r\n");
		addr = FLASH_TEMP_RECORD;
...
//	MSG_LOG("path:%s\r\n",fileName);
	...
	f_lseek(&rfile, length);	//定位到文件最后面
	if(_Fread(recrord, DATALEN, &rfile) != DATALEN){
		f_close(&rfile);
		return 0;		
	}
	...
	if(memcmp(recrord, "银行数据", 8) == 0){//
		MSG_LOG("此为银联数据\r\n");
		
		length = hismsg.offset;
		f_lseek(&rfile, length);	//定位到文件最后面

		if(_Fread(recrord, qPbocRECORD_LEN, &rfile) != qPbocRECORD_LEN){
			f_close(&rfile);
			return 0;		
		}

		read_len = qPbocRECORD_LEN;
