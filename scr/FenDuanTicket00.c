...
extern void DisIcon32(unsigned char x, unsigned char y,unsigned char *disbuf);//显示32*32的图标
...
void SETCpuFDMode(void)
{
	...
	cls();
	display(0, 0, SetDevice, DIS_CENTER);//设置设备类型
	disflag = 1;
...
// 				display(4, 1, ">2.后门机 ", DIS_CONVERT);
				display(2,1,Conn(" 1.",FronDevice),0);//前门机
				display(4,1,Conn(">2.",BackDevice),DIS_CONVERT);//后门机
			}
			else if(newFDMODE == FDMODE_GPS){
// 				display(2, 1, ">1.前门机 ", DIS_CONVERT);
// 				display(4, 1, " 2.后门机 ", 0);
				display(2,1,Conn(">1.",FronDevice),DIS_CONVERT);//前门机
				display(4,1,Conn(" 2.",BackDevice),0);//后门机
			}
			else{
// 				display(2, 1, " 1.前门机 ", 0);
// 				display(4, 1, " 2.后门机 ", 0);
		...
		if(rkey == KEY_ESC){
			...
			if(newFDMODE != gFDMode){
				gFDMode = newFDMODE;
				beep(1, 50, 50);
				display(6, 0, Successfull, DIS_CENTER);//设置成功
				delayxms(800);
				ReStartCPU();
				restore_flag = 3;
				return;
...
void FD_Check_devMode(void)
{
	...
	cls();
#ifdef _AREA_ENG_
		display(0, 0, "Starting.....", DIS_CENTER);
#else
...
void FengDuan_GPS_disp_QM(void)
{
	...
	//		strcpy(disbuff, "位置:");
		strcat(disbuff, LineInfo[gFDderict].StationInfo[gFDStaIndex[gFDderict]].name);
	}
if(strlen(disbuff)<=11)
	display(0,1,disbuff,0);
else
	miniDispstr(0,2,disbuff,0);
	memset(disbuff, 0, 30);
	strcpy(disbuff, Conn(_Font_Line_Str,":"));
	...
	sprintf(disbuff, _Font_Fare_Dis, it/100, it%100);
...
void disMission_HM(void)
{
	...
	case FDProcess_GETVER:
		//strcpy(disbuf, "查询版本中");
		strcpy(disbuf,CheckVer);
		break;
	case FDProcess_GETGPSLineInfo:
#ifdef _AREA_ENG_
		strcpy(disbuf, UpData);
#else
...
//	display(0, 0, " 分段扣费 ", DIS_CENTER|DIS_CONVERT);
	...
	{
		display(2, 1, PositionErr, DIS_CENTER);//未确定位置
		display(4, 1, STOP, DIS_CENTER);
	}
	else if(gGPsPar.EnableGps == 0){//线路信息错误
		display(2, 1, LineInfoErr, DIS_CENTER);
		display(4, 1, STOP, DIS_CENTER);
	}
	else if(gFDRunStatues & FDRUNSTATUES_PJ){//票价信息错误
		display(2, 1, PriceErr, DIS_CENTER);
		display(4, 1, STOP, DIS_CENTER);
	}
	else{
		...
		display(4, 1, SwipOff, DIS_CENTER);
	}
	LED_Dis3("------");
}
...
unsigned int get2stationPrice(unsigned char sta1, unsigned char sta2)
{
	...
	MSG_LOG("CountSum gFDucID:");
	BCD_LOG((unsigned char*)&sta1, 1, 1);
	MSG_LOG("CountSum Cp1->stationNo:");
	BCD_LOG((unsigned char*)&sta2, 1, 1);
	MSG_LOG("CountSum addr:");
	BCD_LOG((unsigned char*)&addr, 1, 1);
	MSG_LOG("CountSum t:");
	BCD_LOG((unsigned char*)&t, 1, 1);
...
// 	if(RunStation.ucDirect <=1){//GPS找到站点,就取这个站点为当前站点。如果没找到就找最近站点。
// 		gFDderict = RunStation.ucDirect;
// 		gFDStaIndex[gFDderict] = RunStation.ucStation;
// 		i  = LineInfo[gFDderict].StationInfo[gFDStaIndex[gFDderict]].ucId;//站点编号 BCD码
// 		gFDucID = (((i>>4)*10)+(i&0x0F));
// 	}
// 	else{//每次都找最近点
		station = 0;
...
// 		display(2,2,"");
	...
	sprintf((char*)buff, Conn(OnBus,":%d"), pcard1->times);
	display(0, 0, (char*)buff, 0);
	if(delMoney != 0){
		if(gchn_eng=='E')
			sprintf((char*)&buff, "Deduct:%d.%02d", delMoney/100, delMoney%100);
		else
			sprintf((char*)&buff, "补扣:%d.%02d", delMoney/100, delMoney%100);
		display(2, 0, (char*)buff, 0);
		if(gchn_eng=='E')
			sprintf((char*)&buff, "Blance:%d.%02d", a_sum1/100, a_sum1%100);
		else
		...
		if(gchn_eng=='E')
			display(3, 0, "Swipe OK", DIS_CENTER);
		else
		    display(3, 0, "刷卡成功!", DIS_CENTER);
		
	}
	if(gchn_eng=='E')
		display(6, 0, "PLS Swip Card Off Bus", 0);
	else
...
void FengDuan_Normal_card(void)
{
	...
	if(pcard1->getupFlag == 0xAA){
		...
		s_sum1=get_s_sum1(0);//取折扣
 		i = 10000;
// 		memcpy((unsigned char*)&i, pcard1->Allprice, 2);
		k=0;
		memcpy((unsigned char*)&k, gFdLinehead.TottalPrice, 2);
		
		if(s_sum1 > i||s_sum1>k){
			MessageBox(1,PriceErr);
			end_card();
			return;
		}
	...
	for(i=0; i<5; i++){//读出卡内分段收费的信息,写入记录区最后一扇区。
		...
		display(6,0,SwipAgain,DIS_CONVERT);//
		while(1){
			if(profile(0) == ST_OK){
				if(old_c_serial != c_serial){
					display(6, 0, SwipSameCard, DIS_CONVERT);
				}
				else{
					break;
				}
	...
	if((gFDderict < 2)&&(gFDStaIndex[gFDderict] < 100))
		strcpy((char*)buff, LineInfo[gFDderict].StationInfo[ucUP_Station-1].name);

	buff[12] = 0;
	strcat((char*)buff, OnBus);//上车
	display(0, 0, (char*)buff, 0);
	if(s_sum1 != 0){
		sprintf((char*)&buff,_Font_KouKuandian, s_sum1/100, s_sum1%100);
		display(2, 0, (char*)buff, 0);
		sprintf((char*)&buff, _Font_Balancedian, a_sum1/100, a_sum1%100);
		display(4, 0, (char*)buff, 0);
	}
	else{
		sprintf((char*)&buff,_Font_KouKuandian, s_sum1/100, s_sum1%100);
		display(2, 0, (char*)buff, 0);
		display(4, 0, PLS_Swipe_OK, 0);
	}
	sprintf((char*)buff, AlsoNtime, pcard1->times);
...
int NoCardShow_ALL(void)
{
	...
	///	sprintf(dspb, "其它错误:%02X", gFDRunStatues);
		sprintf(dspb, Conn(AnOtherErr,":%02X"), gFDRunStatues);
		ret = 6;
	}
	else{
		ret = 0;
	...
	//	sprintf(dspb, "错误:%d", ret);
		sprintf(dspb,Conn(Err,":%d"), ret);
		display(0,0,dspb,0);
		display(4, 0, STOP, DIS_CENTER);
	}
	return ret;
}
//----------------------------------------------------
...
#ifdef BUS_HNHQ_E
	DisIcon32(0,12,(unsigned char*)icon);
#endif
}
