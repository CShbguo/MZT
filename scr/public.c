...
#if KEYB_COM == 1 && defined KEYBOARD//外接键盘 19200
unsigned char DEBUG_COM = COM4;
#elif KEYB_COM == 4 && defined KEYBOARD//外接键盘 19200
unsigned char DEBUG_COM = COM1;
#else
unsigned char DEBUG_COM = COM1;
#endif
//#ifndef _debug_
//#define DEBUG_COM COM1 //COM4
//#else
//#define DEBUG_COM COM1 //COM4
//#endif
...
//建设部对CPU卡的认证指令
unsigned char CpuCardCheck(unsigned char PsamID,unsigned char *pMac, unsigned char *pData, unsigned char len)
{
	...
	IccCommand(PsamID, &Scmd, &Rcmd);		//JSB
...
unsigned char CountMAC(unsigned char *RecBuff)
{
...
#ifdef _debug_
	debugstring("CountTAC	0\r\n");
#endif
	recordd = (DEALRECODE*)RecBuff;
	//初始化缓冲
	memset(recordd->PbocTac, 0x00, 4);
	memset(buffer, 0x00, 16);
	//组织密钥分散数据
	memcpy(buffer, recordd->rPublishNumb , 4);	//发行流水 二级分散因子
	buffer[4]=0xff;
	memcpy(buffer+8, cardMessage.CityCode, 2);	//城市代码 一级分散因子
	buffer[10]=0xff;
	
#ifdef _debug_
	debugstring("PSAMInitForDescrypt(0x44, 0x01, 16, buffer) \r\nbuffer= ");
	...
	//des运算初始化
 	...
 	{
#ifdef _debug_
 		debugstring("PSAMInitForDescrypt error !\r\n");
...
#ifdef _debug_
	debugstring("PSAM MAC(0x05,0,0x28 buffer) \r\nbuffer= ");
	debugdata(buffer,16,1);
#endif
	pos=MAC(gPSAMin,0x05,0,0x28,buffer);
	if(pos!=0)
	{
		memcpy(recordd->PbocTac, buffer, 4);
		return ST_OK;
	}
	else{
#ifdef _debug_
		debugstring("MAC error !\r\n");
		debugdata(buffer, 10, 1);
#endif
		return ST_ERROR;
	}
...
#define SoundPowerOn "[m3],sounda,欢迎使用,三联众瑞智能调度系统\x0"

#define SoundStartST1 "欢迎乘坐\x0"
#define SoundStartST2 "路公交车,本公交车从\x0"
#define SoundStartST3 "出发,开往\x0"
#define SoundStartST4 ",祝您出行愉快\x0"

#define SoundEndST1 "终点站,\x0"
#ifdef BUS_CIFENG_
#define SoundEndST2 "到了.\x0"
#else
#define SoundEndST2 "到了,请带齐随行物品从后门下车\x0"
#endif
...
#else
#define SoundIN2 "到了,到站的乘客请带齐随身物品从后门下车\x0"
#endif
...
void SoundFreeManage(char *sstr)
{
	...
	sprintf(buff, "[v%d]", i);
	strcat(buff, sstr);
	XFcmdsend(0x01, strlen(buff), buff);
}
...
unsigned char SoundCMD;//语音报站任务表 一共保存8个语音 从
...
// 	}
	...
	switch(mode) {
	case IDSoundPowerOn:
		XFcmdsend(XFStartCompose, strlen(SoundPowerOn), SoundPowerOn);
		break;
	case IDSoundStartST:
...
//		i = LineInfo[direct].ucStationMax-1;
		...
		XFcmdsend(XFStartCompose, strlen(strSound), strSound);
		break;
	case IDSoundEndST:
	...
	case IDSoundOUT:
		...
		if(strlen(strtstar) == 0){//没有自定义语音就用默认的。
...
// 		}
	//	strcat(strSound, "soundb|");
		strcat(strSound, strtstar);		//出站，语音尾不能自定义
...
// 		}
	...
	case IDSoundRound:
		...
		XFcmdsend(XFStartCompose, strlen(strSound) , strSound);
		break;
	case IDSoundOverSpeedAT:
		strcpy(strSound, "请注意车速!");
		XFcmdsend(XFStartCompose, strlen(strSound) , strSound);
		break;
	case IDSoundOverSpeed:
		strcpy(strSound, "您已超速!");
		XFcmdsend(XFStartCompose, strlen(strSound) , strSound);
		break;
	case IDSoundOverStation:
		strcpy(strSound, "您已越站.");
		XFcmdsend(XFStartCompose, strlen(strSound) , strSound);
		break;
	default:
		break;
...
//语音报站任务表 一共保存8个语音 从
void SoundAtantionCmd(void)
{	//有限级别  从低位到高位   最低位是报站忙信
	static unsigned char Dly=0;
	
	MSG_LOG("soundCMD:");
	BCD_LOG((unsigned char*)&SoundCMD,1,1);
	if(!(SoundCMD&SoundBusy)||Dly>6)//空闲 或者延时到了
	{//如果忙 就延时6秒再报
		Dly=0;
		if(SoundCMD&SoundOverSpeedAT)//是否有提醒
		{
		...
		else if (SoundCMD&SoundOverSpeed)
		{
...
#endif //
#else //#ifdef _TTSSound_enable_
void SoundManage(unsigned char mode, unsigned char index, unsigned char direct)
{
	return;
}

#endif //#ifdef _TTSSound_enable_
...
unsigned char KeyMange(unsigned char kkvv)
{
	...
	if(kkvv==0x0d)	//确定键
	{
		...
		else
		{
			...
			sysfewrite(BIT_KeyPrice, 4, NewPriceTab.busPrice);	////2014.7.17
			beep(1,500,25);
			KeyDis(KeyVa,Keybuffer);
			LedDisplay(Keybuffer);
			memset(Keybuffer,0x20,5);
...
void debugstringC(char *str, unsigned char mode)
{
	if(mode & 0x80)
		...
		com_snd(DEBUG_COM, strlen(str),(unsigned char *)str);
}
...
// 		KeyPadDis(0x00, i, disbuff, 3); i+=24;
		...
		if(galamessage.style == 0xE0){
			...
			XFcmdsend(XFStartCompose, strlen(disbuff), disbuff);
		}
		else if(galamessage.style == 0xE1)
			k=0x2D;//败
		else// if(galamessage.style == 0xE2)
