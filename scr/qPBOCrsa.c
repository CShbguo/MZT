...
extern void get_BER_TVL(IN INT32U bitPos, OUT BER_TVL *OutTVL);
...
#else
//#define HandListlen 0
unsigned int HandListlen=0;

 unsigned int gHandList[MAX_WHT_BANK_NUM + 1] = {
	0
 };
#endif
...
#define qpoc_long 56
#define qpoc_all_l (0x10000-4)
unsigned char LOAD_qpoc(void)
{
	unsigned char buftemp[100];
	unsigned char DeviceNo_qpoc[10];
	unsigned int addr,i;
	unsigned short len;
	...
	debugstring("加载qpoc\r\n");
#endif
	sysferead(BIT_QPOC_, 2, gBusVerInfo.QPOCVer);
	addr = FLASH_QPOC_;
	memset(buftemp,0,sizeof(buftemp));
	memset(DeviceNo_qpoc,0,sizeof(DeviceNo_qpoc));
	flashread(addr,buftemp,4);
...
addr+=4;
	...
	//	getCpuDeviceNO(pFistVary.DeviceNo);	//重新取设备号
		memcpy(DeviceNo_qpoc,pFistVary.DeviceNo,8);
	//	BCD2Ascii(pFistVary.DeviceNo,DeviceNo_qpoc,4);
		MSG_LOG("本机车牌号：%s\r\n", DeviceNo_qpoc);
		 BCD_LOG(DeviceNo_qpoc,8,1);
      for(i=0;i<qpoc_all_l;i+=qpoc_long)
	  {
		  	MSG_LOG("次数：%d\r\n", i/qpoc_long);

		  flashread(addr+i,buftemp,qpoc_long);
			delayxms(1);
           BCD_LOG(buftemp,8,1);
			if(memcmp(buftemp,DeviceNo_qpoc,8)==0)
			{
				saveMobileParameter(1, buftemp+10);		//商户
				Ascii2BCD("6000090000", buftemp, 10);
				saveMobileParameter(5, buftemp);		//TPDU
				saveMobileParameter(6, buftemp+30);		//设备号
				saveMobileParameter(2, buftemp+40);		//kek明文
				break;
			}
	  }
...
// 			return ST_OK;
	  ...
	  else
	  {
		...
		cls();
		display(3, 0,"更新密钥失败,请检查设备号!", DIS_CENTER);
		delayxms(1500);
	//		return ST_OK;
	//  return ST_ERROR;
	  }
	...
	{
				cls();
		display(3, 0,"没有秘钥文件!", DIS_CENTER);
		delayxms(1500);
	}
}
void save_qpoc_first(unsigned char mode);
unsigned char saveqpoc(unsigned int flash_Addr)
{
	unsigned int ulTemp;
	unsigned int i;
	unsigned char buf[20];
	unsigned int len;
	unsigned char buftemp[500];
...
// #define FLASH_QPOC_ 0x400000		//64K
// #define FLASH_QPOC_END 0x410000

	//保存版本到铁电


	ulTemp = FLASH_QPOC_;              //写白名单flsah“有效”
	
	MSG_LOG("搬移到:0x%08X,", ulTemp);
	flasherase(ERASE64KBYTESECTOR, ulTemp);
	delayxms(50);

	memcpy(buf, "有效", 4);
	flashwrite(ulTemp, buf, 4);
	MSG_LOG("有效GBK:");
	BCD_LOG(buf, 4, 1);
	...
	for (i = 0; i < (len_all / 500); i++)
	{
		flashread(flash_Addr + 12, buftemp, 500);  //22=12+10开头10个字节版本和字节数量
		delayxms(2);
		flashwrite(ulTemp, buftemp, 500);
		ulTemp += 500;
		flash_Addr += 500;

	}
	memset(buftemp, 0, 500);
	flashread(flash_Addr + 12, buftemp, (len_all % 500));
	delayxms(2);
	flashwrite(ulTemp, buftemp, (len_all % 500));
#ifdef _debug_
	debugstring("写完毕\r\n");
	MSG_LOG("len_all:%d\r\n", len_all);
	MSG_LOG("i:%d\r\n", i);
...
#endif
	memcpy(gBusVerInfo.QPOCVer, (unsigned char *)&Ver, 2);
	sysfewrite(BIT_QPOC_, 2, gsl8583FileDownPara.Miss_ver);
	LOAD_qpoc();
	save_qpoc_first(0xad);

	return ST_OK;

}
unsigned char gpoc=0;
unsigned char get_qpoc_first(void)
{
	return gpoc;
...
void qpoc_first(void)
{
	unsigned int addr;
	unsigned char buff[10];
	addr=FLASH_QPOC_;
	
	flashread(addr, buff, 4);
	if (memcmp(buff,"有效",4)==0 )
	{
		save_qpoc_first(0xad);
		MSG_LOG("密钥有效\r\n");
	}	
...
flasherase(ERASE64KBYTESECTOR, FLASH_QPOC_);
	memset(buftemp,0,sizeof(buftemp));
	saveMobileParameter(5, buftemp);		//TPDU
	saveMobileParameter(6, buftemp+30);		//设备号
	saveMobileParameter(2, buftemp+40);		//kek明文
	memset(gBusVerInfo.QPOCVer,0, 2);
	sysfewrite(BIT_QPOC_, 2, gBusVerInfo.QPOCVer);
...
//	借贷记
	...
	PosOfferData.AutenData.bPosCapabilty[2] =CA_DDA;
...
unsigned int sum1_cash_online(unsigned int val)
{
	unsigned int money_1=0;
	unsigned int money_2=0;
	unsigned int money_3=0;

	cardMessage.card_catalog = CARD_QPBOC_BUS;
	cardMessage.Zicard_catalog = 0;
	money_1 = get_s_sum1();//0;//
	cardMessage.card_catalog = CARD_PBOC_CASH;
	cardMessage.Zicard_catalog = 0;
	money_2=get_s_sum1();//0;//
	if (money_1<money_2)
	{
		money_3=money_2;
	}
	...
	if (val>=money_3)
	{
		...
		cardMessage.Zicard_catalog = 0;
		#ifdef _debug_
				MSG_LOG("脱机 DebitMoney:%d\r\n", money_2);
		#endif
		PosOfferData.PDOL.DebitMoney = money_2;
		memcpy(TradeResult.TradeMoney, (INT8U *)&money_2, 4);
		return money_2;
	}
	else
	{
		...
		MSG_LOG("在线 DebitMoney:%d\r\n", money_1);
		#endif
		PosOfferData.PDOL.DebitMoney = money_1;
		memcpy(TradeResult.TradeMoney, (INT8U *)&money_1, 4);
		return money_1;
	}
	...
	for (i = 0; i < AppListCnt; i++) {
		...
		if (ret < 0)
		{
			...
			shuangmian = 1;
						...
						memcpy(TradeResult.TradeMoney, (INT8U *)&s_sum1, 4);
			#endif
		}
#ifdef card_type_qpboc
...
#endif
		{
			...
			if (shuangmian != 0) {
				...
				case 2:
					...
					PosOfferData.PDOL.bPosAttr9F66[0] &= 0xDF;  //1101 1111
					PosOfferData.PDOL.bPosAttr9F66[0] |= 0x20;
		...
		{
			PosOfferData.PDOL.bPosAttr9F66[0] =A1_QPBOC|A1_OFFLINE;//A1_MSD|A1_QPBOC|A1_OFFLINE|A1_SIGN;
			PosOfferData.PDOL.bPosAttr9F66[1] =0;//A2_CVM;
			PosOfferData.PDOL.bPosAttr9F66[2] =0;
			PosOfferData.PDOL.bPosAttr9F66[3] =0;
			online_funtion=0;
		}
		/*
...
//	char tmpCardType = 0;	// 1.存量卡,2.新增卡,3.特定的移动设备卡
	...
	MSG_LOG("t_9F36-5:%04X, %02X, %02X", TempTVL.T, TempTVL.V[0], TempTVL.V[1]);
	BCD_LOG(TempTVL.V, TempTVL.L, 1);
...
//	unsigned char i;
	...
	miniDispstr(7, (21 - strlen((char*)buffer)), (char*)buffer, DIS_RIGHT);
	temp = 0;
...
// 	uiTemp++;
	...
	case E_cash:
		...
		//批次号
		memset(buff,0,sizeof(buff));
		feread(BIT_PBOC_NS_BIG, 4, (unsigned char*)&i);
		sprintf((char*)buff, "%06u", i);
		MSG_LOG("pos流水:%s\r\n",buff);
		memcpy(qrecbuff+uiTemp, ASC2BCD((char*)buff, 6), 3);
		uiTemp += 3;
		break;
...
unsigned char qPbocWriteRecord(unsigned char *rec)
{
	...
	WriteRecord(buffer+RECORD_LEN, 0);
...
start_pos_offline_auten:
	...
	else if(AutenType == TYPE_AUTEN_DDA){		//DDA
